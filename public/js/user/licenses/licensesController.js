(function () {
    ...
    function licensesController($scope, $window, usrService, $cookies, $routeParams) {
        ...
        vm.init = async () => {
            ...
            else vm.licenses = setOrderStatus(licenses.data);

            $scope.$digest();
        };

        /**
         * set a status description to order if it is different to null
         * @param json ddata licenses json
         * @return null
         */
         const setOrderStatus = (data) => {
            for (const iterator of data)
                if (iterator.order != null) {
                    iterator.order.validada == true ? iterator.order.status = 'Orden Validada.' :
                        iterator.order.status = 'Orden por validar.';
                }
